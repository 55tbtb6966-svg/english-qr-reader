...
// v2.2.2 override: PDF-like PowerPoint layout, 1/4/6 cards per slide.
(function(){
  function qrDataUrl(id, size=500){
    const holder=document.createElement("div");
    holder.style.position="fixed";
    holder.style.left="-9999px";
    holder.style.top="-9999px";
    document.body.appendChild(holder);
    new QRCode(holder,{text:id,width:size,height:size,correctLevel:QRCode.CorrectLevel.M});
  ...
  function currentQrRecords(){
    if(window.qrPageSource && window.qrPageSource.length){
      return window.qrPageSource.slice(window.qrPageStart||0,(window.qrPageStart||0)+(window.qrPageSize||10));
    }
    // In the original app these are usually lexical globals, not window props.
    try{
  ...
  function qrFieldFlags(){
    const checked = (id, def) => {
      const el=document.getElementById(id);
      return el ? el.checked : def;
    };
    return {
      id: checked("qrShowId", true),
      english: checked("qrShowEnglish", true),
      ipa: checked("qrShowIpa", true),
      japanese: checked("qrShowJapanese", false),
      japaneseRead: checked("qrShowJapaneseRead", true)
    };
  }
  ...
  function addCenteredText(slide, text, x, y, w, h, fontSize, bold=false, color="222222"){
    if(!text)return;
  ...
  async function exportPptPdfLike(){
    if(typeof pptxgen === "undefined" && typeof pptxgenjs === "undefined" && typeof PptxGenJS === "undefined"){
      setStatus(document.getElementById("qrStatus"),"PowerPoint作成ライブラリを読み込めませんでした。",false);
      return;
    }
    const Pptx = (typeof pptxgen !== "undefined") ? pptxgen : (typeof PptxGenJS !== "undefined" ? PptxGenJS : pptxgenjs);
    const records=currentQrRecords();
    if(!records.length){
      setStatus(document.getElementById("qrStatus"),"先にQRコードを表示してください。",false);
      return;
    }
    const perSlide=Number(document.getElementById("pptPerSlide")?.value || 4);
    const pptx=new Pptx();
    ...
    const flags=qrFieldFlags();
    const {cols, rows}=layoutFor(perSlide);
    const slideW=13.333, slideH=7.5;
    const marginX=0.35, marginY=0.28, gapX=0.18, gapY=0.16;
    const cardW=(slideW - marginX*2 - gapX*(cols-1))/cols;
    const cardH=(slideH - marginY*2 - gapY*(rows-1))/rows;

    for(let s=0; s<records.length; s+=perSlide){
      const slide=pptx.addSlide();
      slide.background={color:"FFFFFF"};
      const page=records.slice(s, s+perSlide);
      ...
      page.forEach((r, i)=>{
        ...
        // light card border like print/PDF cards
        slide.addShape(pptx.ShapeType.roundRect, {
          x, y, w:cardW, h:cardH,
          rectRadius:0.08,
          fill:{color:"FFFFFF", transparency:100},
          line:{color:"C9CED6", width:0.8}
        });

        const qrMax = perSlide===1 ? Math.min(cardW*0.46, cardH*0.52) : Math.min(cardW*0.44, cardH*0.46);
        const qrX=x+(cardW-qrMax)/2;
        const qrY=y+0.12;
        const data=qrDataUrl(r.id, 600);
        if(data){
          slide.addImage({data, x:qrX, y:qrY, w:qrMax, h:qrMax});
        }

        let ty=qrY+qrMax+0.06;
        const lineH=perSlide===1 ? 0.38 : 0.26;
        const fsBase=perSlide===1 ? 22 : perSlide===4 ? 15 : 12;
        if(flags.id){ addCenteredText(slide, r.id, x+0.08, ty, cardW-0.16, lineH, fsBase-5, false, "666666"); ty+=lineH; }
        if(flags.english){ addCenteredText(slide, r.english, x+0.08, ty, cardW-0.16, lineH+0.04, fsBase, true, "111111"); ty+=lineH+0.04; }
        if(flags.ipa && r.ipa){ addCenteredText(slide, r.ipa, x+0.08, ty, cardW-0.16, lineH, fsBase-3, false, "40506A"); ty+=lineH; }
        if(flags.japanese && r.japanese){ addCenteredText(slide, r.japanese, x+0.08, ty, cardW-0.16, lineH, fsBase-2, false, "333333"); ty+=lineH; }
        if(flags.japaneseRead && (r.japaneseRead || r.japanese)){ addCenteredText(slide, r.japaneseRead || r.japanese, x+0.08, ty, cardW-0.16, lineH, fsBase-1, false, "333333"); }
      });
    }

    setStatus(document.getElementById("qrStatus"),"PowerPointを作成中です…");
    await pptx.writeFile({ fileName:`qr_cards_${new Date().toISOString().slice(0,10)}.pptx` });
    setStatus(document.getElementById("qrStatus"),`PowerPointを作成しました（${records.length}件、1枚${perSlide}個）。`);
  }

  window.exportPptPdfLike = exportPptPdfLike;
  window.addEventListener("load",()=>{
    const btn=document.getElementById("pptBtn");
    if(btn){
      // remove previous listener by cloning button
      const newBtn=btn.cloneNode(true);
      btn.parentNode.replaceChild(newBtn, btn);
      newBtn.addEventListener("click", exportPptPdfLike);
    }
  });
})();
